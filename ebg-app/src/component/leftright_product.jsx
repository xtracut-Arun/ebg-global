    ...
    <>
        <div className="container mx-auto">
            {LRcontent.map((item, index) => (
                <div key={index}>
                    <div className={`flex flex-col py-10  ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} items-center `}>
                        <img src={item.img} alt="" className="w-full md:w-[45%]  h-full rounded-[20px]" />
                        <div className='w-full md:w-3/5 py-10  md:p-20'>
                            <p className='dark_grey text-2xl md:text-[42px] leading-none'>{item.title}</p>
